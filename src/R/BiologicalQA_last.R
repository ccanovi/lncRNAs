...
dat <- as.data.frame(log10(counts)) %>% utils::stack() %>% 
  mutate(Stages=samples$Stages[match(ind,samples$SampleID)]) %>% 
  mutate(Batch=samples$Batch[match(ind,samples$SampleID)])

ggplot(dat,aes(x=values,group=ind,col=Stages)) + 
  geom_density() + ggtitle("sample raw counts distribution") +
  scale_x_continuous(name="per gene raw counts (log10)")

ggplot(dat,aes(x=values,group=ind,col=Batch)) + 
  geom_density() + ggtitle("sample raw counts distribution") +
  scale_x_continuous(name="per gene raw counts (log10)")
...
zinb <- zinbwave(se,K=2,epsilon=nrow(counts),
                normalizedValues = TRUE, 
                residuals = TRUE)

dds <- DESeqDataSet(zinb,design= ~ Batch+Stages)
...
data.frame(W, batch=samples$Batch,
           stage=samples$Stages) %>%
  ggplot(aes(W1, W2, colour=stage, shape=batch)) + 
  geom_point(size=3) + 
...
W_new <- as_tibble(reducedDim(zinb))

#' Calculate distance (t-SNE)
tsne_data <- Rtsne(W_new, pca = FALSE, perplexity=9, max_iter=5000)
tsne_data$batch <- samples$Batch
tsne_data$stage <- samples$Stages

#' Plot t-SNE
ggplot(tibble(W1 = tsne_data$Y[,1], W2 = tsne_data$Y[,2], Batch = samples$Batch,
              Stage = samples$Stages), 
       aes(x = W1, y = W2, col = Stage, pch = Batch)) + 
...
p <- ggplot(pc.dat,aes(x=PC1,y=PC2,col=dds$Stages,shape=dds$Batch,text=dds$SampleID)) + 
  geom_point(size=2) + 
